...
class Person:
    ...
    def __repr__(self):
        priorities = ', '.join('{}: {}'.format(k, get_ranking(v)) for (k, v) in self.priorities.items())
...
def main():
    ...
    assignments = {
        p: {
            b: pulp.LpVariable(f'{p.name}_{b}', 0, 1, cat='Integer')
            for b in badges
        }
        for p in people
    }
    ...
    for person in people:
        for badge in badges:
            if person.priority_of(badge) == -1:
                prob += (assignments[person][badge] == 0, f'No {badge} badge for {person.name_}')
    ...
    for person in people:
        print(f'{person.name_}: {person.assignment} ({get_ranking(person.priority_of(person.assignment))})')
